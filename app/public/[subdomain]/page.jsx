...
export default function PublicSalonPage({ params }) {
  ...
  const upiLink = `upi://pay?pa=${upiId}&am=${advance}&cu=INR`;
  ...
  return (
    <div className="min-h-screen">
      {/* Hero */}
      <section className="border-b border-ledger-gold/20 px-6 py-16 text-center">
        <p className="text-xs tracking-wide text-ledger-gold mb-3">{company?.location}</p>
        <h1 className="font-display text-5xl md:text-6xl text-ledger-cream mb-4">{company?.name}</h1>
        <p className="text-ledger-creamDim max-w-md mx-auto">
          Book your appointment below — confirmation and reminders sent straight to WhatsApp.
        </p>
      </section>

      <main className="max-w-[1200px] mx-auto px-6 py-12 grid lg:grid-cols-[1fr_420px] gap-10">
          ...
          {result ? (
            ...
            <form onSubmit={submitBooking} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input type="date" value={form.bookingDate} onChange={(e) => setForm({ ...form, bookingDate: e.target.value })} required />
